...
import {ScreenshotPlugin} from 'react-native-screenshot-plugin';
...
const App = () => {
  ...
  const toggleScreenshot = async () => {
    setLoading(true);
    try {
      const response = await ScreenshotPlugin.toggle(
        isActivated,
      );
  ...
  return (
    <SafeAreaView style={styles.container}>
      <Image source={require('./src/assets/app-logo.png')} style={{height: 100, width: 100}} />
      ...
      <View style={styles.statusContainer}>
        <Text
          style={[styles.statusText, isActivated ? styles.activated : styles.deactivated]}>
          Status : {isActivated ? 'Activated' : 'Deactivated'}
        </Text>
      </View>

      <TouchableOpacity
        style={[styles.toggleButton, isActivated ? styles.deactivateBtn : styles.activateBtn]}
